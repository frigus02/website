...
func (i *staticFileItem) update(file *fs.File) error {
	...
	if origBase == file.Name || strings.HasPrefix(origBase, ".") {
		// Don't use hashes in top level or dot files. These usually rely on
		// their exact name, e.g. ".htaccess" or "favicon.ico".
		i.name = file.Name
	} else {
